...
BU升级的时候遇到的一个问题：相同的jar包，在同一批机器中有的机器报异常，有的机器正常，报错信息是NoClassDefFoundError

#### 报错原因

在加载BU时，bu的静态代码块中调了BuLoader的方法，在BuLoader的静态代码块中，调了这个方法，在这里面调了线程的getcontextClassLoader方法，然后用这个classloader进行操作，而这个classloader是null导致的，最终导致BU加载失败。
...
1.怀疑是jar包冲突，jar包冲突处理完成后依然有这个问题

2.分析报错信息，发现是类加载的初始化的阶段报的错，而真正引起类加载报错的原因的堆栈信息只有第一次类加载的时候才会抛出来，之后只会抛出来: Could not initialize class的异常信息，然后我们发布大量的机器，复现第一次类加载错误的原因

3.怀疑是CompletableFuture.supplyAsync，异步线程造成的，本地单元测试排查原因发现复现失败，Tomcat中却**偶发**

#### 具体原因

1. 在Tomcat中使用CompletableFuture.supplyAsync来第一次加载Bu，调Bu的静态代码块时，拿到的ForkJoinWorkerThread的contextClassLoader为null
2. Tomcat的server组件在init的时候，设置ForkjoinPool的ForkJoinWorkerThreadFactory为tomcat内置的SafeForkJoinWorkerThreadFactory，而SafeForkJoinWorkerThreadFactory新建线程时设置contextClassLoader的类加载器是加载ForkJoinPool的类加载器为null

#### 疑问

**为什么有的机器没有问题，有的机器有问题**

1. 没有问题的原因是：没有使用CompletableFuture.supplyAsync来第一次加载Bu，例如在Tomcat的处理请求的线程池中加载Bu，处理请求的线程的contextClassLoader是UrlClassLoader，在RPC线程池中加载BU，处理请求的线程的contextClassLoader是ParallelWebappClassLoader，都不是null，所有没有问题
2. 有问题的原因：使用CompletableFuture.supplyAsync来第一次加载Bu，之后加载BU的时候直接就报错啦

**改为同步可以吗**

1. 不可以：不使用CompletableFuture.supplyAsync来第一次加载Bu，例如在Tomcat的处理请求的线程池中加载Bu，处理请求的线程的contextClassLoader是UrlClassLoader，在RPC线程池中加载BU，处理请求的线程的contextClassLoader是ParallelWebappClassLoader，都不是null，但是后续加载可能有问题
...
**ForkJoinPool.class.getClassloader为null原因**

1. 加载ServletContainerInitializer、filter、servlet、listener类加载器都是ParallelWebappClassLoader
2. 处理请求的线程的contextClassLoader是UrlClassLoader（commonClassLoader）
3. 由于没有指定类加载器时，类的加载器使用的是加载该类的加载器；在DispatcherServlet的init方法中，初始化Spring的单例池，所以容器中的Bean的类加载器默认是ParallelWebappClassLoader（在不指定类加载器的情况下）
4. 所以在Tomcat中几乎所有的类加载器都是ParallelWebappClassLoader（在不指定类加载器的情况下），ParallelWebappClassLoader加载类的时候，先从缓存中看这个类是否加载过，没有加载过会使用ext ClassLoader进行类加载，利用双亲委派机制，只用bootstrap ClassLoader这个类加载器来加载ForkJoinPool，bootstrap ClassLoader在jvm中拿到的为 null
...
临时解决方案：机器发布后，首先手动调一个同步的RPC接口进行Bu类加载

最终解决方案：利用Spring容器初始化的扩展点ApplicationListener在容器启动时先同步加载Bu类
