...
4. 所以在Tomcat应用中能拿到的类加载器几乎都是ParallelWebappClassLoader（除了JRE的类以及Tomcat的相关的类，在不指定类加载器的情况下），ParallelWebappClassLoader加载类的时候，先从缓存中看这个类是否加载过，没有加载过会使用ext ClassLoader进行类加载，利用双亲委派机制，只用bootstrap ClassLoader这个类加载器来加载ForkJoinPool，bootstrap ClassLoader在jvm中拿到的为 null
